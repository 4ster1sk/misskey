...
```
To keep the test data isolated from your development environment, the E2E setup uses a dedicated `redis-test` service and a separate `cherrypick_test` database on the shared `db` server. This script copies `.config/playwright-devcontainer.yml` to `.config/test.yml`, migrates the test database, then starts the test server and runs Cypress.
...
AI(LLM)によるレビューやコーディング補助時にハルシネーションのもとになるので、AIが誤解・混乱しそうなコードは避けるかコメントを残そう。

AIにとって分かりやすいコードは、結果的に人間にとっても分かりやすいコードに繋がります。
