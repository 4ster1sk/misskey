...
describe('admin/delete-account', () => {
	...
	test('管理者が一般ユーザーを削除できる', async () => {
		const target = await signup({ username: 'delete_target1' });
	...
	test('他の管理者が一般ユーザーを削除できる', async () => {
		const target = await signup({ username: 'delete_target2' });
		await successfulApiCall({
			endpoint: 'admin/delete-account',
			parameters: {
				userId: target.id,
	...
	test('モデレーターが削除を試みると ROLE_PERMISSION_DENIED エラーになる', async () => {
		const target = await signup({ username: 'delete_target3' });
		await failedApiCall({
			endpoint: 'admin/delete-account',
			parameters: {
				userId: target.id,
	...
	test('一般ユーザーが削除を試みると ROLE_PERMISSION_DENIED エラーになる', async () => {
		const target = await signup({ username: 'delete_target4' });
		await failedApiCall({
			endpoint: 'admin/delete-account',
			parameters: {
				userId: target.id,
	...
	test('ログインしていないユーザーが削除を試みると CREDENTIAL_REQUIRED エラーになる', async () => {
		const target = await signup({ username: 'delete_target5' });
		await failedApiCall({
			endpoint: 'admin/delete-account',
			parameters: {
				userId: target.id,
	...
	test('削除済みユーザーの削除は成功する', async () => {
		const target = await signup({ username: 'delete_target6' });
		await api('admin/delete-account', { userId: target.id }, root);
